...
/**
 * Solves the sudoku by trying out every possible number.
 * This can take long but can solve every valid Sudoku.
 * The algorithm operates on a nested Array instead of the Sudoku class.
 */
public class BacktrackingAlgorithm extends AAlgorithm {
     ...
     * @param candidates - the Sudoku array.
    ...
    private boolean isNumberAllowedInField(int number, int rowIndex, int columnIndex,
        ...
        return ( isNumberAllowedInRow(number, rowIndex, candidates)
              && isNumberAllowedInColumn(number, columnIndex, candidates)
              && isNumberAllowedInSquare(number, rowIndex, columnIndex, candidates)     
        );
    }
